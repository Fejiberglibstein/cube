import { Gradient, Layout, Path, makeScene2D } from "@motion-canvas/2d";
import {
	Color,
	PossibleColor,
	SimpleSignal,
	...
	const blurs = paths.map((v, i) => {
		...
		return range(blurAmount).map((height, j) => (
			<Path
				...
				ref={refs}
				data={v}
				fill={getGradient(i, refs[0], "#704cad")}
				lineJoin={"round"}
...
function getGradient(index: number, p: Path, color: PossibleColor) {
		...
		case 8:
			return new Gradient({
				...
				stops: [
					{ offset: 0, color: c },
					{ offset: 0.5, color: c },
					{ offset: 0.5, color: c.darken() },
					{ offset: 1, color: c.darken() },
				],
			});
	}
}
