...
export default makeScene2D(function* (view) {
  ...
  const heights: SimpleSignal<SimpleSignal<number>[]> = createSignal(
    new Array<SimpleSignal<number>>(),
  );
  const lines = createRef<Layout>();
  const colors = ['#8b34a7', '#3a5c93 ', 'green', 'yellow', 'orange', 'red'];
  let faceHeight: number;
  paths.forEach((_, i) => {
    heights().push(createSignal(0));
  });
  ...
  yield* all(
    ...
    ...cubies.map((cubie, i) => {
      ...
      return all(
        ...
        chain(
          waitFor(delay),
          heights()[i](faceHeight / 1.2 / 10, duration / 1.2, easeOutBack),
          heights()[i](0, duration / 1.4, easeOutExpo),
        ),
      );
    }),
  );
