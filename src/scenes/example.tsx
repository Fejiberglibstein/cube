...
export default makeScene2D(function* (view) {
  ...
  const [wrapper, side] = [cubies, leftSide, rightSide].map(
    ref =>
      (
        <Layout>
          {paths.map((v, i) => (
    ...
    return range(lineResolution).map((height, j) => (
      <Path
        ...
        fill={getGradient(
          i,
          bbox,
          Color.lerp('#704cad', '234d9e', height / lineResolution, 'lab'),
        )}
        lineJoin={'round'}
  ...
  yield* all(
    ...
    ...cubies.map((cubie, i) => {
      ...
      return chain(
        waitFor(delay),
        all(
          cubie.y(-cubie.y() - faceHeight, duration, easeInOutSine),
          cubie
            .fill('#dddddd', duration / 0.6, easeInOutSine)
            .to({r: 19, g: 19, b: 21, a: 1}, 0),
          sequence(
            duration * 0.58,
            heightsTop()[i](lineResolution, duration, easeInOutSine),
            heightsBottom()[i](lineResolution, duration, easeInOutSine),
          ),
        ),
      );
    }),
